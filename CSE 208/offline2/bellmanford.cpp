...
public:
	...
	bool bellman_ford(int s);//will run dfs in the graph
};
...
void Graph::Relax(int u,int k)
{
    int v=adjList[u].getItem(k);

    if(dist[v]>dist[u]+adjList[u].getweight(k))
    {

        dist[v]=dist[u]+adjList[u].getweight(k);
        parent[v]=u;
    }
}
 ...
 for(int u=0;u<nVertices-1;u++)
     {
         for(int j=0;j<adjList[u].getLength();j++)
         {
             ...
             if(dist[v]>dist[u]+adjList[u].getweight(j))
             {
                 printf("There is no solution\nThere exists negative weighted cycle\n");
                return false;
             }
